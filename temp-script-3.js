        ...
        function backToConflictChoice() {
            // Step: "No" on confirmation → go back to Reschedule/Cancel choice
            const confirmModal = document.getElementById('conflictConfirmModal');
            if (confirmModal) { confirmModal.classList.add('hidden'); confirmModal.classList.remove('flex'); }
            const choiceModal = document.getElementById('conflictNotifModal');
        ...
        function renderEquipmentGrid(list) {
            ...
            grid.innerHTML = list.map(item => {
                // Sanity check: available should never exceed total quantity logically in the UI
                ...
                if (item.isLocked) {
                    ...
                    actionBtn = '<button onclick="openBorrowModalWithEquip(' + item.id + ')" class="px-4 py-2 bg-gradient-to-r from-emerald-500 to-teal-500 hover:from-emerald-600 hover:to-teal-600 text-white text-xs font-bold rounded-xl transition transform hover:scale-105 shadow-lg cursor-pointer border-none">📝 Borrow</button>';
                } else {
                    actionBtn = '<span class="text-xs font-bold text-red-500">Out of stock</span>';
                }

                // Pending badge — show if other users have pending requests
                const pendingBadge = (item.pending && item.pending > 0)
                    ? '<div class="mt-2 flex items-center gap-1.5 px-2.5 py-1.5 bg-amber-50 border border-amber-200 rounded-lg">' +
                      '<span class="w-1.5 h-1.5 bg-amber-500 rounded-full animate-pulse flex-shrink-0"></span>' +
                      '<span class="text-xs font-semibold text-amber-700">⏳ ' + item.pending + ' unit' + (item.pending > 1 ? 's' : '') + ' with pending requests from other users</span></div>'
                    : '';
                ...
                return '<div class="group relative overflow-hidden rounded-2xl p-5 bg-white dark:bg-gray-800 border-2 ' + (ok && !item.isLocked ? 'border-emerald-200 hover:border-emerald-400 hover:shadow-xl transition-all duration-300 hover:-translate-y-1' : 'border-gray-100 opacity-70') + '">' +
                    '<div class="absolute top-4 right-4 z-10 max-w-[50%]"><span class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs font-bold shadow-sm whitespace-nowrap overflow-hidden text-ellipsis ' + (ok ? statusBg + ' ' + statusColor : 'bg-gray-100 text-gray-500') + '"><span>' + statusIcon + '</span> ' + item.available + ' available</span></div>' +
                    '<div class="flex items-start gap-4 mb-4 mt-2 relative z-0">' + getEquipmentIcon(item.name) + 
                    '<div class="flex-1 pt-1 pr-[85px]"><h4 class="font-bold text-lg text-gray-800 dark:text-white leading-tight mb-1">' + item.name + '</h4><p class="text-xs text-gray-500 font-medium">' + (item.category || 'General') + '</p></div></div>' +
                    ...
                    pendingBadge +
                    '<div class="flex items-center justify-between pt-2 mt-2 border-t border-gray-100"><div class="flex items-center gap-2"><span class="text-xs font-medium text-gray-500">' + item.quantity + ' total</span></div>' +
                    actionBtn +
                    '</div></div>';
            }).join('');
        }
        ...
        async function loadMyBorrowingsList() {
            ...
            container.innerHTML = sorted.map(b => {
                ...
                return '<div class="group relative overflow-hidden rounded-2xl p-5 bg-white dark:bg-gray-800 border-2 ' + statusBorder + ' shadow-sm hover:shadow-md transition-all duration-300">' +
                    ...
                    rejectionMsg +
                    (b.status === 'pending' ? '<div class="mt-4 pt-4 border-t border-gray-100 flex items-center justify-between"><p class="text-xs text-amber-600 font-medium flex items-center gap-1">⏳ Waiting for approval</p><div class="flex gap-2"><button onclick="openEditBorrowingModal(' + b.id + ')" class="px-4 py-2 rounded-lg text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 transition-colors">✏️ Edit</button><button onclick="cancelEqRequest(' + b.id + ')" class="px-4 py-2 rounded-lg text-xs font-bold text-red-600 bg-red-50 hover:bg-red-100 transition-colors">Cancel</button></div></div>' : '') +
                    '</div>';
            }).join('');
        }
        ...
        async function openBorrowModalWithEquip(equipId) {
            ...
            const pendingNote = (item.pending && item.pending > 0)
                ? ' &nbsp;<span style="color:#b45309;font-size:11px;font-weight:600;">⚠️ ' + item.pending + ' unit' + (item.pending > 1 ? 's' : '') + ' pending from other users</span>'
                : '';
            document.getElementById('borrowMaxHelp').innerHTML = '📦 Max: ' + item.available + ' units available' + pendingNote;
        ...
        function updateBorrowDateDisplays() {
            ...
            if (borrowReturnDate) {
                ...
                if (rangeDisplay) rangeDisplay.innerHTML = '<span class="text-emerald-600">📅 ' + fullMonths[start.getMonth()] + ' ' + start.getDate() + ' → ' + fullMonths[d.getMonth()] + ' ' + d.getDate() + ', ' + d.getFullYear() + '</span> <span class="ml-2 px-2 py-1 bg-emerald-500 text-white rounded-lg text-xs font-bold">' + diffDays + ' day' + (diffDays > 1 ? 's' : '') + '</span>';
            } else {
                if (rDisp) {
        ...
        async function loadConcernsView() {
            ...
            container.innerHTML = sorted.map(c => {
                ...
                if (actualDescription.includes('[ATTACHED_IMAGE_DATA]')) {
                    ...
                    actualDescription = parts[0].trim();
        ...
        function switchVenue(venue) {
            ...
            if (venue === 'basketball') {
                if (titleEl) titleEl.innerHTML = '📅 Court Booking';
                if (subtitleEl) subtitleEl.textContent = 'Reserve the barangay basketball court.';
            } else {
                if (titleEl) titleEl.innerHTML = '🏢 Hall Booking';
            ...
            if (activeBookings.length === 0) { 
                ...
                myContainer.innerHTML = activeBookings.map(b => {
                    ...
                    return '<div class="border rounded-xl p-4 shadow-sm" style="background-color: var(--panel-bg); border-color: var(--border-color);">' +
                        ...
                        (b.status === 'pending' ? '<button onclick="openEditCourtBookingModal(' + b.id + ')" class="w-full py-1.5 text-xs font-bold text-blue-600 bg-blue-50 hover:bg-blue-100 rounded transition">✏️ Edit Booking</button>' : '') +
        ...
        async function openDaySchedulePopup(dateStr) {
            ...
            if (window.pendingRescheduleData) {
                ...
                document.getElementById('dsToggleFormBtn').innerHTML = '<span>＋ Add Court Booking</span>';
                let banner = document.getElementById('dsRescheduleBanner');
        ...
        async function cancelMyReservation(id) {
            // Enforce 3-cancellation limit
            const allBookings = await getCourtBookings();
            const uid = String(user.id);
            const myBookings = allBookings.filter(b => String(b.user_id) === uid || String(b.userId) === uid);
            const cancelledCount = myBookings.filter(b => b.status === 'cancelled' && !b.admin_comment).length;
            if (cancelledCount >= 3) {
                showToast('You have reached the 3-cancellation limit. You can book again tomorrow.', 'error');
                return;
            }
            if (!await showConfirmModal('Are you sure you want to cancel this reservation? This counts toward your 3-cancellation limit.', 'Cancel Reservation', 'Yes, Cancel', 'No', 'warning')) return;
        ...
        async function cancelEqRequest(id) {
            // Enforce 3-cancellation limit for equipment
            const allBorrowings = await getMyBorrowings();
            const cancelledEqCount = (allBorrowings || []).filter(b => b.status === 'cancelled').length;
            if (cancelledEqCount >= 3) {
                showToast('You have reached the 3-cancellation limit for equipment requests.', 'error');
                return;
            }
            if (!await showConfirmModal('Cancel this equipment request? (' + cancelledEqCount + '/3 cancellations used)', 'Cancel Request', 'Yes, Cancel', 'No', 'warning')) return;
            const res = await cancelCourtBooking(id); // borrowings cancel shares same pattern
            if (res.success) {
                showToast('Equipment request cancelled.', 'success');
        ...
        async function loadHistoryView() {
            ...
            container.innerHTML = '<p class="text-gray-500 italic text-sm">Loading history...</p>';
            try {
                ...
                let all = [];
                bor.forEach(b => all.push({ type: 'Borrowing', title: `Borrowed: ${b.equipment_name || 'Equipment'} (x${b.quantity})`, date: new Date(b.created_at || b.date_requested), status: b.status, icon: '📦', color: 'blue' }));
                con.forEach(c => all.push({ type: 'Concern', title: `Reported: ${c.title || c.category}`, date: new Date(c.created_at), status: c.status, icon: '💬', color: 'amber' }));
                boo.forEach(b => all.push({ type: 'Court Booking', title: `Booked: ${b.venue === 'basketball' ? 'Basketball Court' : 'Multi-Purpose Hall'}`, date: new Date(b.created_at || (b.date + 'T' + b.time)), status: b.status, icon: '📅', color: 'emerald' }));
                all.sort((a,b) => b.date - a.date);
                if (all.length === 0) { container.innerHTML = '<p class="text-gray-500 italic text-[15px] font-bold mt-4">No activity history found yet. If you believe this is an error, please try logging out and in again.</p>'; return; }
                container.innerHTML = all.map(a => {
                    ...
                    return `<div class="flex items-start gap-4 p-4 rounded-xl border border-gray-100 hover:shadow-md transition bg-gray-50/50 dark:bg-slate-800/50 dark:border-slate-700"><div class="w-12 h-12 rounded-full flex items-center justify-center text-2xl bg-${a.color}-100 text-${a.color}-600 shrink-0">${a.icon}</div><div class="flex-1"><div class="flex justify-between items-start flex-wrap gap-2"><h4 class="font-bold text-gray-800 dark:text-gray-100 text-sm">${a.title}</h4><span class="text-xs font-bold px-2 py-1 rounded-md ${statusColor}">${displayStatus}</span></div><div class="flex items-center gap-3 mt-2 text-xs text-gray-500 font-medium"><span class="bg-gray-200 dark:bg-slate-700 px-2 py-0.5 rounded text-gray-600 dark:text-gray-300">${a.type}</span><span>${isNaN(a.date.getTime())?'Unknown Date':a.date.toLocaleDateString()+' '+a.date.toLocaleTimeString([],{hour:'2-digit',minute:'2-digit'})}</span></div></div></div>`;
                }).join('');
        ...
        async function loadDashboardTOTPStatus() {
            ...
            if (!actionsEl) return;
            actionsEl.innerHTML = '<p style="color:var(--text-muted);font-size:13px;">Loading...</p>';
