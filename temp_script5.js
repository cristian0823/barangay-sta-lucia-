if (!requireAuth()) window.location.href = 'login.html';
            ...
            document.addEventListener('DOMContentLoaded', async function () {
                ...
                // Run clock immediately Ã¢â‚¬â€ before any awaits
                initWelcomeClock();
            ...
            async function loadSystemStatsForProfile() {
                ...
                document.getElementById('statBookingsAdminInfo').textContent = bookings.length;
            ...
            function adminSelectVenue(venue) {
                ...
                if (venue === 'basketball') {
                    ...
                    venueDisplay.innerHTML = 'Ã°Å¸Ââ‚¬ Basketball Court';
                    ...
                    venueDisplay.innerHTML = 'Ã°Å¸ÂÂ¢ Multi-Purpose Hall';
            ...
            document.getElementById('adminSidebarEventForm')?.addEventListener('submit', async function (e) {
                ...
                msgBox.innerHTML = result.success
                    ? '<div class="bg-green-100 text-green-700 p-3 rounded">Ã¢Å“â€¦ Event Created</div>'
                    : '<div class="bg-red-100 text-red-700 p-3 rounded">Ã¢ÂÅ’ ' + result.message + '</div>';
                ...
                if (result.success) {
                    ...
                    try {
                        broadcastEmailToAllResidents(
                            `ðŸ“… New Barangay Event: ${eventData.title}`,
                            `A new barangay event has been scheduled.`,
                            `ðŸ“ ${eventData.location || 'Barangay Sta. Lucia'} | ðŸ“† ${eventData.date}${eventData.time ? ' at ' + eventData.time : ''}${eventData.description ? ' | ' + eventData.description : ''}`
                        );
                    ...
                    try {
                        ...
                        if (conflicting.length > 0) {
                            let warningHtml = `<div style="margin-top:16px;background:#fffbeb;border:1.5px solid #fde68a;border-radius:14px;padding:16px;">
                                <div style="font-size:14px;font-weight:700;color:#92400e;margin-bottom:8px;">Ã¢Å¡Â Ã¯Â¸Â ${conflicting.length} existing booking(s) conflict with this event:</div>
                            ...
                            conflicting.forEach(b => {
                                warningHtml += `<div style="font-size:12px;background:#fff;border:1px solid #fde68a;border-radius:8px;padding:8px 10px;color:#78350f;">
                                    Ã°Å¸â€˜Â¤ <strong>${b.userName || 'Resident'}</strong> Ã¢â‚¬â€ ${b.timeRange || b.time}
                                </div>`;
                            });
                            warningHtml += `</div>
                                <button id="notifyConflictBtn" onclick="notifyConflictingUsers(${JSON.stringify(conflicting).replace(/"/g, '&quot;')}, '${eventData.title.replace(/'/g, "\\'")}')"
                                    ...
                                    Ã°Å¸â€œÂ£ Notify All Affected Users
                                </button>
                            </div>`;
            ...
            async function notifyConflictingUsers(bookings, eventTitle) {
                ...
                try {
                    ...
                    for (const b of bookings) {
                        ...
                        // 1. Fully delete the conflicting booking from the database so it disappears completely
                    ...
                    if (btn) { btn.textContent = 'Ã¢Å“â€¦ Cancelled & Notified!'; btn.style.background = '#16a34a'; }
                    ...
                    if (btn) { btn.textContent = 'Ã¢ÂÅ’ Failed'; btn.disabled = false; }
            ...
            document.getElementById('adminBookingForm')?.addEventListener('submit', async function (e) {
                ...
                document.getElementById('adminBookingMessage').innerHTML = result.success
                    ...
                    : '<div class="bg-red-100 text-red-700 p-3 rounded">⚠️ ' + result.message + '</div>';
            ...
            async function renderAdminCalendar() {
                ...
                try {
                    ...
                    function isBookingTimePassed(dateStr, timeStr) {
                        const todayStr = new Date().toLocaleDateString('en-CYYYY-MM-DD local
                    ...
                    const todayStr = new Date().toLocaleDateString('en-C                _e.filter(e => e.status === 'approved' && e.date >= todayStr).forEach(e => {
                ...
                if (rightList) {
                    ...
                    if (combined.length === 0) {
                        rightList.innerHTML = `<div style="text-align:center;padding:32px 16px;">
                            <div style="font-size:40px;margin-bottom:12px;">Ã°Å¸â€œÂ­</div>
                        ...
                        rightList.innerHTML = combined.map(item => {
                            ...
                            const dot = isEv ? 'Ã°Å¸Å¸Â£' : 'Ã°Å¸â€Â´';
                            ...
                            return `<div style="background:var(--panel-bg, #f8fafc);border:1px solid var(--border, #e2e8f0);border-radius:12px;padding:12px;display:flex;gap:12px;align-items:flex-start;">
                                ...
                                <div>
                                    ...
                                    <div style="font-weight:700;font-size:11px;color:${isEv ? '#a855f7' : '#ef4444'};">${dateFmt} Ã¢â‚¬Â¢ ${item.time}</div>
                                </div>
                            </div>`;
                        }).join('');
                    }
                        ...
                        try {
                            ...
                            if (allUsers && allUsers.length > 0) {
                                const notifPayloads = allUsers.map(u => ({
                                    user_id: u.id,
                                    type: 'event_added',
                                    message: `Ã°Å¸â€œÂ¢ New Barangay Event: "${eventName}" on ${new Date(adminSelectedDate + 'T00:00:00').toLocaleDateString('en-US', { month: 'long', day: 'numeric', year: 'numeric' })}. Court Reservations on this day have been cancelled.`,
            ...
            function openAdminCancelBookingsEventFlow() {
                ...
                if (btn) btn.innerHTML = '<span>Ã¢â€“Â² Hide Form (Cancelling Bookings)</span>';
            ...
            function openAdminJustAddEventFlow() {
                ...
                if (formWrap) formWrap.style.display = 'block';
                if (btn) btn.innerHTML = '<span>Ã¢â€“Â² Hide Form</span>';
            ...
            function switchSection(section, btn) {
                ...
                const allSections = ['overview', 'court-bookings', 'multipurpose-bookings', 'requests', 'concerns', 'events', 'equipment', 'users', 'reports', 'activity-log', 'profile', 'system'];
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬ Activity Log Filter State Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
            ...
            function renderActivityTable() {
                ...
                tbody.innerHTML = filtered.map(log => {
                    ...
                    return `<tr>
                        ...
                        <td data-label="User / Admin" style="text-align: center;"><div class="admin-cell"><span class="admin-icon">Ã°Å¸â€˜Â¤</span> <strong>${log.adminUsername || 'System'}</strong></div></td>
            ...
            async function loadUsers() {
                ...
                if (!tbody) return;
                tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:20px;color:#9ca3af;">Loading...</td></tr>';
                ...
                tbody.innerHTML = users.map(u => {
                    ...
                    const roleColor = u.role === 'admin' ? '#059669' : '#6b7280';
                    const roleBadge = `<span style="background:${u.role === 'admin' ? '#d1fae5' : '#f3f4f6'};color:${roleColor};padding:3px 10px;border-radius:50px;font-size:12px;font-weight:600;">${u.role}</span>`;
                    
                    const isSuspended = u.suspended_until && new Date(u.suspended_until) > new Date();
                    const suspensionBadge = isSuspended ? `<span style="background:#fee2e2;color:#dc2626;padding:3px 10px;border-radius:50px;font-size:11px;font-weight:600;margin-left:5px;">Suspended Ã°Å¸â€â€™</span>` : '';
                    
                    const displayName = u.fullName || u.full_name || u.username;
                    const phone = u.phone || u.contact_number || u.contactNumber || 'Ã¢â‚¬â€';

                    return `<tr style="cursor:pointer;" onclick="openUserProfileModal(${u.id})" class="hover-row">
                    <td style="font-size:12px;font-weight:700;color:var(--green-xl);">${u.barangay_id || 'Ã¢â‚¬â€'}</td>
                    <td>
                        <strong>${displayName}</strong>
                        ${isMe ? ' <span style="font-size:11px;color:#10b981;">(You)</span>' : ''}
                        ${suspensionBadge}
                    </td>
                    <td style="font-size:13px;">${phone}</td>
                    <td style="font-size:13px;color:#6b7280;">${u.email || 'Ã¢â‚¬â€'}</td>
                    <td>${roleBadge}</td>
                    <td onclick="event.stopPropagation()">
                        ${!isMe && u.role !== 'admin' ? `
                            ${isSuspended
                                ? `<button class="btn btn-smaller" style="background-color:#10b981; color:white; padding:4px 10px; border-radius:6px; font-weight:600; font-size:12px; margin-right:5px; border:none; cursor:pointer;" onclick="liftSuspension(${u.id}, '${u.username}')">Ã¢Å“â€¦ Lift Suspension</button>`
                                : `<button class="btn btn-smaller" style="background-color:#fbbf24; color:white; padding:4px 10px; border-radius:6px; font-weight:600; font-size:12px; margin-right:5px; border:none; cursor:pointer;" onclick="openSuspendModal(${u.id}, '${u.username}', ${u.offense_count || 0})">Ã°Å¸â€â€™ Suspend</button>`
                            }
                            <button class="btn btn-smaller btn-danger" style="padding:4px 10px; border-radius:6px; font-weight:600; font-size:12px; border:none; cursor:pointer;" onclick="adminDeleteUserConfirm(${u.id}, '${u.username}')">Delete</button>
                        ` : ''}
                    </td>
                </tr>`;
                }).join('');
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬ BATCH UPLOAD FUNCTIONS Ã¢â€â‚¬Ã¢â€â‚¬
            let _batchRows = [];

            function handleBatchFileDrop(e) {
                const file = e.dataTransfer.files[0];
                if (file) previewBatchCSV(file);
            }

            function previewBatchCSV(file) {
                if (!file) return;
                ...
                reader.onload = function(e) {
                    ...
                    for (let i = 1; i < lines.length; i++) {
                        ...
                        previewBody.innerHTML += `<tr>
                            <td style="font-size:12px;">${row.barangay_id||''}</td>
                    ...
                    document.getElementById('batchPreview').style.display = _batchRows.length > 0 ? 'block' : 'none';
                    document.getElementById('batchRowCount').textContent = `(${_batchRows.length} residents)`;
                    document.getElementById('batchUploadBtn').disabled = _batchRows.length === 0;
            ...
            async function submitBatchUpload() {
                ...
                try {
                    ...
                    resultBox.textContent = `Ã¢Å“â€¦ Done! ${inserted} resident(s) uploaded. ${skipped > 0 ? skipped + ' skipped (duplicate or missing data).' : ''}`;
                    ...
                    resultBox.textContent = 'Ã¢ÂÅ’ Upload failed: ' + err.message;
                    btn.disabled = false;
                    btn.textContent = 'Upload Residents';
                }
            }
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬ Instant row-update helpers (no full table reload) Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
            ...
            function _updateReqRow(id, newStatus) {
                ...
                if (cells[6]) {
                    if (newStatus === 'approved') {
                        cells[6].innerHTML = `<button onclick="returnRequest(${id})" style="padding:6px 12px;background:#f1f5f9;color:#475569;border:1px solid #cbd5e1;border-radius:8px;font-size:12px;font-weight:700;cursor:pointer;">Ã°Å¸â€â„¢ Returned</button>`;
                    } else {
                        cells[6].innerHTML = '<span style="font-size:12px;color:#9ca3af;padding:6px 0;">Ã¢â‚¬â€</span>';
                    }
                }
            }
            function _updateBookingRow(id, newStatus, reason) {
                ...
                if (cells[6]) cells[6].innerHTML = '<span style="font-size:12px;color:#9ca3af;">Ã¢â‚¬â€</span>';
            }
            function _updateConcernRow(id, newStatus, assignee, hasResponse) {
                ...
                if (cells[3] && hasResponse && !cells[3].querySelector('.replied-note')) {
                    cells[3].insertAdjacentHTML('beforeend', '<div class="replied-note" style="margin-top:4px;font-size:11px;color:#059669;font-style:italic;">Ã¢Å“â€¦ Replied</div>');
                }
            }
                ...
                container.innerHTML = bookings.map(booking => {
                    ...
                    <div class="court-booking-card">
                        <div style="display: flex; justify-content: space-between; align-items: start; margin-bottom: 16px;">
                            <div>
                                <h4 style="margin: 0; color: var(--dark); font-size: 16px; font-weight: 700;"><i class="bi bi-calendar-event-fill"></i> ${formatDate(booking.date)}</h4>
                                <p style="margin: 6px 0 0; color: var(--gray); font-size: 14px;">Ã¢ÂÂ° ${booking.time}</p>
                        ...
                        <div style="margin-bottom: 12px;">
                            <p style="margin: 4px 0; color: #555; font-size: 14px;"><strong>Ã°Å¸â€˜Â¤ Resident:</strong> ${booking.userName}</p>
                            <p style="margin: 4px 0; color: #555; font-size: 14px;"><strong>Ã°Å¸â€œÂ Purpose:</strong> ${booking.purpose}</p>
                        </div>
                        
                        ${booking.adminComment ? `
                            <div style="background: #fffbeb; border-left: 4px solid #f59e0b; padding: 10px; border-radius: 4px; margin-bottom: 12px;">
                                <p style="margin: 0; color: #92400e; font-size: 13px;"><strong>Ã°Å¸â€™Â¬ Admin Reply:</strong> ${booking.adminComment}</p>
                            </div>
                        ` : ''}
                        
                        ${booking.replies && booking.replies.length > 0 ? `
                            <div style="margin-bottom: 12px;">
                                <p style="margin: 0 0 8px; color: #555; font-size: 13px; font-weight: 600;">Ã°Å¸â€™Â¬ Replies:</p>
                                ${booking.replies.map(reply => `
                                    <div style="background: #f9fafb; padding: 8px; border-radius: 4px; margin-bottom: 6px;">
                                        <div style="display: flex; justify-content: space-between; font-size: 11px; color: #666; margin-bottom: 4px;">
                                            <span>Ã°Å¸â€˜Â¤ ${reply.userName}</span>
                        ...
                        ${booking.status !== 'cancelled' && booking.status !== 'rejected' ? `
                            <div style="margin-bottom: 16px;">
                                <label style="display: block; margin-bottom: 8px; font-size: 13px; color: var(--gray); font-weight: 600;">Ã°Å¸â€™Â¬ Send Reply / Explanation:</label>
                        ...
                        if (b) {
                            ...
                            if (email) {
                                await sendEmailNotification({
                                    to_email: email, name,
                                    title: 'Court Reservation Approved âœ…',
                    ...
                    try {
                        ...
                        if (b) {
                            ...
                            if (email) {
                                await sendEmailNotification({
                                    to_email: email, name,
                                    title: 'Court Reservation Update â Œ',
            ...
            async function deleteCourtBooking(bookingId) {
                if (await showConfirmModal('Are you sure you want to permanently delete this Court Reservation?', 'Delete Booking', 'Yes, Delete', 'Cancel', 'danger')) {
                    const result = await (typeof window.deleteBooking === 'function' ? window.deleteBooking(bookingId) : supabase.from('court_bookings').delete().eq('id', bookingId));
            ...
            async function confirmRejectEq() {
            ...
            async function loadEvents() {
                ...
                if (events.length === 0) {
                    container.innerHTML = '<div class="empty-state"><div class="empty-state-icon">Ã°Å¸Ââ‚¬</div><p>No events</p></div>';
                    return;
                }
            ...
            async function loadEquipment() {
                ...
                const EQUIP_ICON_MAP = {
                    'Chairs': { html: `Ã°Å¸Âªâ€˜`, cls: 'eq-Chairs' },
                    ...
                    'Tents': { html: `Ã¢â€ºÂº`, cls: 'eq-Tents' },
                ...
                container.innerHTML = equipment.map(e => {
                    ...
                    let statusBadge;
                    if (e.isArchived) {
                        statusBadge = '<span class="status-badge" style="background:#f3f4f6;color:#374151;border:1px solid #d1d5db;"><i class="bi bi-box-fill"></i> Archived</span>';
                    } else if (broken > 0 && e.available === 0) {
                        statusBadge = '<span class="status-badge" style="background:#fff7ed;color:#c2410c;border:1px solid #fed7aa;">Ã°Å¸â€Â§ Under Repair</span>';
                    } else if (broken > 0) {
                        statusBadge = '<span class="status-badge" style="background:#fff7ed;color:#c2410c;border:1px solid #fed7aa;">Ã°Å¸â€Â§ Partial/Repair</span>';
                    } else if (e.available === 0) {
                        statusBadge = '<span class="status-badge status-borrowed">Ã°Å¸â€œÂ¤ In Use</span>';
                    } else if (inUse > 0) {
                        statusBadge = '<span class="status-badge status-pending">Ã°Å¸â€â€ž Partially Used</span>';
                    } else {
                        statusBadge = '<span class="status-badge status-approved">Ã¢Å“â€¦ Available</span>';
                    }
                    const iconDef = EQUIP_ICON_MAP[e.name];
                    const iconHtml = iconDef ? iconDef.html : (e.icon || 'Ã°Å¸â€œÂ¦');
            ...
            async function generateBookingsCSV() {
                ...
                downloadCSV(convertToCSV(data), 'court_bookings_report.csv');
                showAlert('Bookings CSV Exported', 'success');
            }
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬ Mobile Sidebar Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
            ...
            async function printEquipmentReport() {
                ...
                openPrintWindow('Barangay Sta. Lucia Ã¢â‚¬â€ Equipment & Asset Report',
            ...
            async function printBookingsReport() {
                ...
                const rows = data.map(b => `<tr><td>${b.userName}</td><td>${b.venueName || b.venue || 'N/A'}</td><td>${b.date}</td><td>${adminFmt12(b.time)}${b.end_time ? ' Ã¢â‚¬â€œ ' + adminFmt12(b.end_time) : ''}</td><td>${b.purpose}</td><td><strong>${b.status}</strong></td></tr>`).join('');
                openPrintWindow('Barangay Sta. Lucia Ã¢â‚¬â€ Facility Reservations Report',
            ...
            async function printConcernsReport() {
                ...
                openPrintWindow('Barangay Sta. Lucia Ã¢â‚¬â€ Citizen Complaints Report',
            ...
            async function printBorrowingsReport() {
                ...
                openPrintWindow('Barangay Sta. Lucia Ã¢â‚¬â€ Equipment Borrowings Report',
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬ Live Polling for booking sections Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬Ã¢â€â‚¬
            ...
            async function loadMultipurposeBookings() {
                ...
                allBookings = allBookings.filter(b => {
                    ...
                    if (b.date === currentIsoDate) {
                        try {
                            ...
                            if(parts.length === 1) parts = timeStr.split('Ã¢â‚¬â€œ');
                ...
                tbody.innerHTML = mpBookings.map(b => {
                    ...
                    const timeStr = b.timeRange || b.time || 'Ã¢â‚¬â€';
                    ...
                    return `<tr data-bid="${b.id}" onclick="openBookingRespond(${b.id})" style="cursor:pointer;transition:background 0.15s;" onmouseover="this.style.background='rgba(16,185,129,0.06)'" onmouseout="this.style.background=''">
                        ...
                        <td style="color:var(--text);">${b.purpose || 'Ã¢â‚¬â€'}</td>
                        <td>${statusBadge}</td>
                    </tr>`;
                }).join('');
            }
            ...
            async function loadAdminBookings() {
                ...
                tbody.innerHTML = allBookings.map(b => {
                    ...
                    <td style="color:var(--text);">${b.date}</td>
                    <td style="color:var(--text);">${timeStr}</td>
                    <td style="color:var(--text);">${b.purpose || 'Ã¢â‚¬â€'}${adminComment}</td>
                    <td>${statusBadge}</td>
                </tr>`;
                }).join('');
            }
            ...
            async function approveAdminBooking(bookingId) {
                if (!await showConfirmModal('Approve this booking?', 'Approve Booking', 'Approve', 'Cancel', 'info')) return;
                try {
            ...
            // Ã¢â€â‚¬Ã¢â€â‚¬ WELCOME CLOCK & GREETING Ã¢â€â‚¬Ã¢â€â‚¬
            ...
            async function loadOverview() {
                ...
                if (feed) {
                    try {
                        ...
                        const actionIcons = {
                            'Login': 'Ã°Å¸â€Â', 'Borrow Request': 'Ã°Å¸â€œÂ¦', 'Borrow Approved': 'Ã¢Å“â€¦',
                            'Borrow Rejected': 'Ã¢ÂÅ’', 'Borrow Returned': 'Ã°Å¸â€â„¢', 'Borrow Cancelled': 'Ã°Å¸Å¡Â«',
                            'Court Reservation Submitted': 'Ã°Å¸Ââ‚¬', 'Booking Approved': 'Ã¢Å“â€¦', 'Booking Rejected': 'Ã¢ÂÅ’',
                            'Concern Submitted': 'Ã°Å¸â€œâ€¹', 'Concern Updated': 'Ã°Å¸â€œÂ', 'Event Created': 'Ã°Å¸â€œâ€¦',
                            'Equipment Added': 'Ã°Å¸Âªâ€˜', 'Inventory Update': 'Ã°Å¸â€Â§', 'User Registered': 'Ã°Å¸â€˜Â¤',
                            'User Deleted': 'Ã°Å¸â€”â€˜Ã¯Â¸Â'
                        };
                        if (recent.length === 0) {
            ...
            async function loadAdminNotifications() {
                ...
                body.innerHTML = notifications.map(notif => {
                    ...
                    const icon = notif.type === 'concern' ? 'Ã°Å¸â€œâ€¹' : notif.type === 'borrow' ? 'Ã°Å¸â€œÂ¦' : 'Ã°Å¸Ââ‚¬';
            ...
            async function openAdminDayPopup(dateStr) {
                const venue = document.getElementById('adminSelectedVenue').value;
                const venueLabel = venue === 'basketball' ? 'Ã°Å¸Ââ‚¬ Basketball Court' : 'Ã°Å¸ÂÂ¢ Multi-Purpose Hall';
            ...
            async function refreshAdminDaySchedule(dateStr, venue) {
                ...
                dayBookings.forEach(b => {
                    const timeStr = adminFmt12(b.time) + (b.end_time ? ' Ã¢â‚¬â€œ ' + adminFmt12(b.end_time) : '');
                    entries.push({ type: 'booking', id: b.id, timeStr, label: b.userName || b.username || 'Resident', sub: b.purpose || '' });
                });
                dayEvents.forEach(e => {
                    const timeStr = adminFmt12(e.time) + (e.end_time ? ' Ã¢â‚¬â€œ ' + adminFmt12(e.end_time) : '');
                ...
                if (entries.length === 0) {
                    ...
                    list.innerHTML = entries.map(en => {
                        ...
                        const dot = isBk ? 'Ã°Å¸â€Â´' : 'Ã°Å¸Å¸Â£';
                        ...
                        return `<div style="display:flex;align-items:flex-start;gap:12px;padding:12px;border-radius:12px;border:1.5px solid ${border};background:${bg};margin-bottom:8px;">
                            ...
                            <button onclick="${removeFn}" style="flex-shrink:0;padding:4px 10px;border-radius:8px;border:1.5px solid #e5e7eb;background:#fff;font-size:11px;font-weight:700;color:#ef4444;cursor:pointer;"><i class="bi bi-x-lg"></i> Remove</button>
                        </div>`;
                    }).join('');
                }
                ...
                slots.forEach(slot => {
                    ...
                    for (const b of allBookings) {
                        if (b.date === dateStr && b.status !== 'rejected' && b.status !== 'cancelled' &&
                            ...
                            let [st, et] = tRange.split('Ã¢â‚¬â€œ').map(s => s.trim());
                            if (!et) et = st;
            ...
            window.handleAdsEventSubmit = async function (e) {
                ...
                if (!dateStr || !venue) {
                    btn.disabled = false; btn.textContent = 'Ã°Å¸Å½â€° Schedule Event';
                ...
                btn.disabled = false; btn.textContent = 'ðŸŽ‰ Schedule Event';
            ...
            async function loadConcerns() {
                ...
                try {
                    ...
                    if (!concerns.length) {
                        tbody.innerHTML = '<tr><td colspan="8" style="text-align:center;padding:32px;color:#9ca3af;">ðŸ“‹ No concerns submitted yet</td></tr>';
                        return;
                    }
                    ...
                    tbody.innerHTML = concerns.map(c => {
                        ...
                        const submittedAt = c.createdAt || c.created_at ? new Date(c.createdAt || c.created_at).toLocaleString('en-US', {month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit'}) : 'Ã¢â‚¬â€';
                        const residentDisplay = `<div style="line-height:1.2;"><strong>${c.userName || 'Ã¢â‚¬â€'}</strong><br><small style="color:#6b7280;font-size:11px;">${submittedAt}</small></div>`;
                        const date = c.createdAt ? new Date(c.createdAt).toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }) : 'Ã¢â‚¬â€';
                        const responseNote = c.response ? `<div style="margin-top:4px;font-size:11px;color:#059669;font-style:italic;">Ã¢Å“â€¦ Replied</div>` : '';
                        const hasImage = c.description && c.description.includes('[ATTACHED_IMAGE_DATc.imageUrl;
                        const imgBadge = hasImage ? `<span style="margin-left:6px;font-size:10px;background:#dbeafe;color:#1d4ed8;padding:2px 7px;border-radius:20px;font-weight:700;">Ã°Å¸â€œÂ· Photo</span>` : '';
                        ...
                        <td>${residentDisplay}</td>
                        <td>${c.address || 'Ã¢â‚¬â€'}</td>
                        <td>${c.category || 'Ã¢â‚¬â€'}</td>
                        <td>${c.title || 'Ã¢â‚¬â€'}${responseNote}${imgBadge}</td>
                        <td>${date}</td>
                    </tr>`;
                    }).join('');
                } catch (e) {
                    console.error("loadConcerns crash:", e);
                    tbody.innerHTML = `<tr><td colspan="8" style="text-align:center;padding:32px;color:#ef4444;font-weight:bold;">Ã¢Å¡Â Ã¯Â¸Â Error loading concerns:<br/><span style="font-size:12px;font-weight:normal;">${e.message}</span></td></tr>`;
                }
            }
            ...
            function openConcernRespond(concernId) {
                ...
                if (headerSub && concern) {
                    headerSub.textContent = `${concern.userName || 'Resident'} Ã‚Â· ${concern.category || ''} Ã‚Â· ${concern.status}`;
                }
                ...
                if (detailsDiv && concern) {
                    ...
                    if (actualDesc.includes('[ATTACHED_IMAGE_DAT                       const parts = actualDesc.split('[ATTACHED_IMAGE_DAT                     actualDesc = parts[0].trim();
                    ...
                    let html = `
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin-bottom:16px;">
                            <div style="background:var(--panel-bg);border-radius:10px;padding:12px;border:1px solid var(--border);">
                                ...
                                <p style="margin:0;font-size:14px;font-weight:700;color:var(--text);">${concern.userName || 'Ã¢â‚¬â€'}</p>
                            </div>
                            <div style="background:var(--panel-bg);border-radius:10px;padding:12px;border:1px solid var(--border);">
                                ...
                                <p style="margin:0;font-size:14px;font-weight:600;color:var(--text);">${concern.address || 'Ã¢â‚¬â€'}</p>
                            </div>
                            <div style="background:var(--panel-bg);border-radius:10px;padding:12px;border:1px solid var(--border);">
                                ...
                                <p style="margin:0;font-size:14px;font-weight:600;color:var(--text);">${concern.category || 'Ã¢â‚¬â€'}</p>
                            </div>
                            <div style="background:var(--panel-bg);border-radius:10px;padding:12px;border:1px solid var(--border);">
                        ...
                        <div style="margin-bottom:16px;background:var(--panel-bg);border-radius:10px;padding:14px;border:1px solid var(--border);">
                            ...
                            <p style="margin:0;font-size:15px;font-weight:700;color:var(--text);">${concern.title || 'Ã¢â‚¬â€'}</p>
                        </div>
                        <div style="margin-bottom:16px;background:var(--panel-bg);border-radius:10px;padding:14px;border:1px solid var(--border);">
                    ...
                    if (imageUrl) {
                        html += `
                            <div style="margin-bottom:16px;">
                                <p style="margin:0 0 8px;font-size:11px;font-weight:700;color:#9ca3af;text-transform:uppercase;letter-spacing:0.5px;">Ã°Å¸â€œÂ· Attached Photo</p>
                ...
                // No response form to populate; readonly mode.
            ...
            function openBookingRespond(bookingId) {
                ...
                if (headerSub) {
                    headerSub.textContent = `${booking.userName || 'Resident'} Ã¢â‚¬Â¢ ${booking.venueName || booking.venue || 'Facility'} Ã¢â‚¬Â¢ ${booking.status}`;
                }
                ...
                if (actionsDiv) {
                    ...
                    if (isPending) {
                        actionsDiv.innerHTML = `
                            <button onclick="approveAdminBooking(${booking.id}); closeAdminBookingModal();" style="flex:1;padding:12px;background:#059669;color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">Ã¢Å“â€¦ Approve Booking</button>
                            <button onclick="openRejectModal(${booking.id}); closeAdminBookingModal();" style="flex:1;padding:12px;background:#ef4444;color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">Ã¢ÂÅ’ Reject Booking</button>
                        `;
                    } else {
                        actionsDiv.innerHTML = `
                            <p style="width:100%;font-size:13px;color:var(--muted);margin-bottom:16px;text-align:center;">This booking has already been completely processed.</p>
            ...
            function openRequestRespond(reqId) {
                ...
                if (headerSub) {
                    headerSub.textContent = `${req.userName || 'Resident'} Ã¢â‚¬Â¢ ${req.equipment || 'Item'} Ã¢â‚¬Â¢ ${req.status}`;
                }
                ...
                if (actionsDiv) {
                    if (req.status === 'pending') {
                        actionsDiv.innerHTML = `
                            <button onclick="approveRequest(${req.id}); closeAdminRequestModal();" style="flex:1;padding:12px;background:#059669;color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">Ã¢Å“â€¦ Approve Details</button>
                            <button onclick="rejectRequest(${req.id}); closeAdminRequestModal();" style="flex:1;padding:12px;background:#ef4444;color:#fff;border:none;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">Ã¢ÂÅ’ Reject</button>
                        `;
                    } else if (req.status === 'approved') {
                        actionsDiv.innerHTML = `
                            <button onclick="returnRequest(${req.id}); closeAdminRequestModal();" style="flex:1;padding:12px;background:#f1f5f9;color:#475569;border:1px solid #cbd5e1;border-radius:12px;font-size:14px;font-weight:700;cursor:pointer;">Ã°Å¸â€â„¢ Mark as Returned</button>
                        `;
                    } else {
                        actionsDiv.innerHTML = `
